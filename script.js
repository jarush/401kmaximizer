...
$(document).ready(() => {
  ...
  chartInstance = new Chart(ctx, {
    ...
    data: {
      ...
      datasets: [
        {
          label: 'Cumulative Pre-Tax Contributions ($)',
          data: [],
          borderColor: '#2b6cb0',
    ...
    options: {
      ...
      scales: {
        y: {
          beginAtZero: true,
          max: 25500, // Safe baseline height starting frame configuration
          grid: { color: '#e2e8f0' }
        },
...
function calculateStrategy() {
  ...
  $('#val-remaining-cap').text(`$${Math.max(0, remainingPretaxCap).toLocaleString(undefined, {maximumFractionDigits: 2})}`);
  ...
  for (let i = 0; i < totalPaychecks; i++) {
    ...
    chartDataTotal.push(Math.min(limitPretax, runningPretax));
  }
  ...
  const projectedEmployerMatch = (ytdGross + (remainingPaychecks * gross)) * (companyMatch / 100);
  const totalProjectedAllSources = runningPretax + ytdPosttax + projectedEmployerMatch;
  if (totalProjectedAllSources > limitCombined) {
    $ul.append($('<li>').addClass('text-danger').text(`Warning: Total projected contributions ($${totalProjectedAllSources.toLocaleString(undefined, {maximumFractionDigits:0})}) exceed the IRS Combined Annual Limit.`));
  }

  $('#schedule-text').html($ul);

  renderChart(chartLabels, chartDataTotal, currentPaycheckIndex, currentPaycheckIndex + switchPaycheckIndex, limitPretax);
}
...
function renderChart(labels, data, historyLimit, switchIdx, maxLimit) {
  if (!chartInstance) {
    return;
  }

  const maxLimitLineData = Array(labels.length).fill(maxLimit);
  const pointColors = data.map((v, i) => i === switchIdx ? '#dd6b20' : (i < historyLimit ? '#718096' : '#2b6cb0'));
  ...
  // Update the IRS limit
  chartInstance.data.datasets[1].data = maxLimitLineData;
  chartInstance.options.scales.y.max = maxLimit + 1000;
